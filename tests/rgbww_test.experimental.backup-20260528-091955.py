...
def request(method, url, data=None):
    ...
    try:
        ...
        raise Exception(
            "HTTP {} {} failed: {} (host={}) payload={}"
            .format(method, url, error.reason, host, request_text[:300])
        )

def do_post(queryParams, post_data):
    url = u"http://{}/{}".format(host, queryParams)
    r = request(u"POST", url, data=post_data)
    if (r.status_code != 200):
        raise Exception(
            "HTTP POST failed: status={} url={} host={} body={} payload={}"
            .format(r.status_code, url, host, r.text[:500], str(post_data)[:500])
        )
...
def set_channel_cmd(cmd, channels="'h','s','v'"):
    ...
    if (r.status_code != 200):
        raise Exception(
            "HTTP POST failed: status={} cmd={} host={} body={} payload={}"
            .format(r.status_code, cmd, host, r.text[:500], post_data[:500])
        )
    return ts
